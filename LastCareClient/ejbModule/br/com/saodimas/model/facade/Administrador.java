...
public class Administrador implements AdministradorRemote {
	...
	public void atualizarDadosCartao(AssociadoVO associado)
			throws SQLException, MensagemSaoDimasException {

		Connection conn = null;
		try {
			associado.setQtdeCartao(associado.getQtdeCartao() + 1);
			associado.setDataUltimaImpressao(new Date());
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildAssociadoDAO().atualizaDadosImpressaoCartao(associado, conn);
	...
	public void deleteCompra(CompraVO compra)
		...
		try {
			...
			throw new MensagemSaoDimasException(
					"Não foi possível remover a compra.");
		} finally {
	...
	public void deleteConta(ContaVO conta) throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildContaDAO().delete(conta, conn);
		} catch (MySQLIntegrityConstraintViolationException cve) {
			throw new MensagemSaoDimasException("Não foi possível remover a conta.");
		} finally {
	...
	public void deleteObito(ObitoVO obito) throws SQLException,
		...
		try {
			...
			throw new MensagemSaoDimasException(
					"Não foi possível remover o obito.");
		} finally {
	...
	public void deleteApolice(ApoliceVO apolice) throws SQLException,
		...
		try {
			...
			cve.printStackTrace();
			throw new MensagemSaoDimasException(
					"Não foi possível remover a apolice.");
	...
	public void deleteAssociado(AssociadoVO associado) throws SQLException,
		...
		try {
			...
			throw new MensagemSaoDimasException(
					"Não foi possível remover o associado.");
	...
	public List<ClienteVO> listConsultaClientes(String tipo,
			String[] parametros, String status) throws SQLException,
			MensagemSaoDimasException {
		Connection conn = null;
		try {
			conn = SqlConnector.getInstance().getConnection();
			return DAOFactory.DEFAULT.buildClienteDAO().consultaClientes(tipo,
					parametros, status, conn);
...
/*
	...
	/**\
	 * estornar uma fatura que foi dada baixa errada.
	 * @param vo
	 * @throws SQLException
	 * @throws MensagemSaoDimasException
	 */
	public void estornarBaixaFatura(FaturaVO vo)
			throws SQLException, MensagemSaoDimasException {
		...
		try {
			...
			DAOFactory.DEFAULT.buildFaturaDAO().estornarBaixaFatura(vo, conn);
	...
	public List<VendaVO> listVendasByCliente(ClienteVO cliente)
			throws SQLException, MensagemSaoDimasException {

		Connection conn = null;
		try {
			conn = SqlConnector.getInstance().getConnection();
			return DAOFactory.DEFAULT.buildVendaDAO().getAllVendasByCliente(cliente, conn);
	...
	@Override
	public VendaVO insertVenda(VendaVO venda) throws SQLException,
			MensagemSaoDimasException {
		Connection conn = null;
		try {
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildVendaDAO().save(venda, conn);
			
			List<ProdutoVendaVO> listProd = venda.getProdutos();
			if(listProd != null){
				for (ProdutoVendaVO prod : listProd) {
					prod.setVenda(venda);
					DAOFactory.DEFAULT.buildProdutoVendaDAO().save(prod, conn);
				}
			}
			
			List<ServicoVendaVO> listS = venda.getServicos();
			if(listProd != null){
				for (ServicoVendaVO prod : listS) {
					prod.setVenda(venda);
					DAOFactory.DEFAULT.buildServicoVendaDAO().save(prod, conn);
				}
			}
			
			List<ParcelaVO> listP = venda.getParcelas();
			if(listP != null){
				for (ParcelaVO par : listP) {
					par.setVenda(venda);
					DAOFactory.DEFAULT.buildParcelaDAO().save(par, conn);
				}
			}
	...
	@Override
	public VendaVO updateVenda(VendaVO venda) throws SQLException,
			MensagemSaoDimasException {
		Connection conn = null;
		try {
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildVendaDAO().update(venda, conn);
			List<ProdutoVendaVO> listaVenda = venda.getProdutos();
			
			for (ProdutoVendaVO produtoVenda : listaVenda) {
				if (produtoVenda.getId() != null)
					DAOFactory.DEFAULT.buildProdutoVendaDAO().update(produtoVenda, conn);
				else {
					produtoVenda.setVenda(venda);
					DAOFactory.DEFAULT.buildProdutoVendaDAO().save(produtoVenda, conn);
				}
			}
			
			List<ServicoVendaVO> listaServicos = venda.getServicos();

			for (ServicoVendaVO servicoVenda : listaServicos) {
				if (servicoVenda.getId() != null)
					DAOFactory.DEFAULT.buildServicoVendaDAO().update(servicoVenda, conn);
				else {
					servicoVenda.setVenda(venda);
					DAOFactory.DEFAULT.buildServicoVendaDAO().save(servicoVenda, conn);
				}
			}


			venda = DAOFactory.DEFAULT.buildVendaDAO().findById(venda.getId(),conn);
			return venda;
	...
	
	{
			...
			try {
				conn = SqlConnector.getInstance().getConnection();
				DAOFactory.DEFAULT.buildVendaDAO().finalizarVenda(venda, ListasComuns.STATUS_VENDA[1], conn);
	...
	@Override
	public void deleteVenda(VendaVO venda) throws SQLException,
			MensagemSaoDimasException {
		Connection conn = null;
		try {
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildVendaDAO().delete(venda, conn);
		} catch (MySQLIntegrityConstraintViolationException cve) {
			cve.printStackTrace();
	...
	public List<VendaVO> listVendas() throws SQLException,
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			return DAOFactory.DEFAULT.buildVendaDAO().findAll(conn);
	...
	public void deleteProdutosVenda(List<ProdutoVendaVO> list)
		...
		try {
			...
			for (ProdutoVendaVO vo : list) {
				DAOFactory.DEFAULT.buildProdutoVendaDAO().delete(vo, conn);
			}
	...
	@Override
	public void deleteServicosVenda(List<ServicoVendaVO> list)
			throws SQLException, MensagemSaoDimasException {
		Connection conn = null;
		try {

			conn = SqlConnector.getInstance().getConnection();
			for (ServicoVendaVO vo : list) {
				DAOFactory.DEFAULT.buildServicoVendaDAO().delete(vo, conn);
			}
	...
	public List<FormaPagamentoVO> listFormaPagamento() throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			return DAOFactory.DEFAULT.buildFormaPagamentoDAO().findAll(conn);
	...
	public List<ParcelamentoVO> listParcelamentoByFormaPagamento(FormaPagamentoVO formaPagamento) throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			return DAOFactory.DEFAULT.buildParcelamentoDAO().getAllParcelamentoByFormaPagamento(formaPagamento, conn);
	...
	public List<ParcelaVO> listParcelaByVenda(VendaVO venda) throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			return DAOFactory.DEFAULT.buildParcelaDAO().getAllParcelasByVenda(venda, conn);
	...
	public List<ContaVO> listContas() throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			return DAOFactory.DEFAULT.buildContaDAO().findAll(conn);
	...
	public List<TipoContaVO> listTipoContas() throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			return DAOFactory.DEFAULT.buildTipoContaDAO().findAll(conn);
	...
	public List<BancoVO> listBancos() throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			return DAOFactory.DEFAULT.buildBancoDAO().findAll(conn);
	...
	public ContaVO updateConta(ContaVO conta) throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildContaDAO().update(conta, conn);
			return conta;
	...
	public ContaVO insertConta(ContaVO conta) throws SQLException, MensagemSaoDimasException {
		...
		try {
			...
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildContaDAO().save(conta, conn);
			return conta;
	...
	public List<ChequeVO> listCheques() throws SQLException, MensagemSaoDimasException {
		...
		try {
			...
			conn = SqlConnector.getInstance().getConnection();
			return DAOFactory.DEFAULT.buildChequeDAO().findAll(conn);	
	...
	public ChequeVO insertCheque(ChequeVO cheque) throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildChequeDAO().save(cheque, conn);
			return cheque;
		} finally {
	...
	public void deleteCheque(ChequeVO cheque) throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildChequeDAO().delete(cheque, conn);
	...
	public ChequeVO updateCheque(ChequeVO cheque) throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildChequeDAO().update(cheque, conn);
			return cheque;
	...
	public ChequeVO baixarCheque(ChequeVO cheque, Date dataBaixa) throws SQLException, MensagemSaoDimasException {
		...
		try {
			conn = SqlConnector.getInstance().getConnection();
			DAOFactory.DEFAULT.buildChequeDAO().baixarCheque(cheque, dataBaixa, conn);
			return cheque;
