...
public class ApoliceEditarInfoPanel extends JPanel {
	...
	public void atulizadaDadosApolice() throws MensagemSaoDimasException {
		...
		{	
			if(!ValidaDatas.isDataValida(spnDataSuspensao.getText()))
				erro = "Data de suspensão invalida. Alterações não foram salvas.";
			else{
		...
		}else if(apolice.getStatus().equals(ListasComuns.STATUS_APOLICE[2]))
		{
			if(!ValidaDatas.isDataValida(spnDataCancelamento.getText()))
				erro = "Data de cancelamento invalida. Alterações não foram salvas.";
			else{
	...
	private void initialize() {
		...
		lbDataSuspensao = new JLabel("Data Suspensão:  ", JLabel.RIGHT);
		...
		lbMotivoSuspensao = new JLabel("Motivo Suspensão:  ", JLabel.RIGHT);
	...
	private void configure() {
		...
		paneObsOcultas.setBorder(BorderFactory.createTitledBorder("Observações Ocultas"));
		...
		if (apolice != null && plano != null) {
			...
			if (apolice.getDependentes() != null
				...
				c.mostrarMensagemDependente(
						"Há mais dependentes no apolice do que o plano selecionado permite.",
