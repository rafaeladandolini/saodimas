...
public class ConsultaApolicePanel extends JPanel {
	...
	private static final int OBSERVACAO = 8;
	private static final int PLANO = 9;
	private static final String[] OPCOES_CONSULTA = {"Qualquer","N. Apólice","Nome do Titular","Documento","Data de Adesão", "Dependente", "Cidade", "Endereço", "Observação", "Plano"}; 
	...
	private void initialize() {
		...
		lbPlano.setVisible(false);
		
		List<PlanoVO> listP = Controller.getInstance().getPlanoAll();
		PlanoVO[] arrayP = new PlanoVO[listP.size()];
		
		cbPlano = new JComboBox(listP.toArray(arrayP));
		cbPlano.setSelectedIndex(1);
		cbPlano.setPreferredSize(DFIELDM);
		cbPlano.setMinimumSize(DFIELDM);
		cbPlano.setVisible(false);
	...
	private void configure() {
		...
		c.gridy = 6; infApolice.add(lbCidades, c);
		c.gridy = 7; infApolice.add(lbEndereco, c);
		c.gridy = 8; infApolice.add(lbObservacao, c);
		c.gridy = 9; infApolice.add(lbPlano, c);
		c.gridy = 10; infApolice.add(lbStatus, c);
		...
		c.gridy = 6; infApolice.add(cbCidades, c);
		c.gridy = 7; infApolice.add(txtEndereco, c);
		c.gridy = 8; infApolice.add(txtObservacao, c);
		c.gridy = 9; infApolice.add(cbPlano, c);
		c.weighty = 1;
		c.gridy = 10; infApolice.add(panelStatus, c);
	...
	private String[] getParamentros()
	{
		...
		else if(cbOpcoes.getSelectedItem().toString().equals(OPCOES_CONSULTA[PLANO]))
			p.add(((PlanoVO)cbPlano.getSelectedItem()).getId().toString());
	...
	private class EventoComboOpcoes implements ItemListener{
		public void itemStateChanged(ItemEvent e) {
			...
			boolean showPlano	    = e.getItem() != OPCOES_CONSULTA[QUALQUER] && e.getItem() == OPCOES_CONSULTA[PLANO];
