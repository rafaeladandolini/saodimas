...
public class ApoliceEndContatoPanel extends JPanel {
	...
	private JTextArea txtCompEndereco;
	...
	private void initialize() {
		...
		lbCompEndereco = new JLabel("Complemento: *", JLabel.RIGHT);
		lbCompEndereco.setPreferredSize(DLABEL);
		lbCompEndereco.setMinimumSize(DLABEL);
		
		txtCompEndereco = new JTextArea();
		txtCompEndereco.setPreferredSize(new Dimension(150, 100));
		txtCompEndereco.setBorder(BorderFactory.createLineBorder(Color.GRAY));
	...
	private void configure() {
		...
		c.gridy = 5; infEndereco.add(lbCompEndereco, c);
		...
		c.gridy = 5; infEndereco.add(txtCompEndereco, c);
		c.weighty = 1;
		c.gridy = 6; infEndereco.add(new JLabel(), c);
...
//				btErroCidade.setMensagem(ValidaApolice.validaCidade((Cidade)cbCidade.getSelectedItem()));
				...
				if (erros.length() > 0) barNotificacao.mostrarMensagem("Erro ao submeter os dados! Clique no botão de notificação para exibir a mensagem!", BarraNotificacao.AVISO);
					...
					apolice.setCompEndereco(txtCompEndereco.getText().trim());
