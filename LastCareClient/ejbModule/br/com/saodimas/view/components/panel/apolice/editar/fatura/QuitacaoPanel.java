...
public class QuitacaoPanel extends JPanel{
    ...
    private JComboBox<String> comboSemCarne;
	...
	public void setFaturasTableModel(FaturaTableModel faturasTableModel) {
		...
		comboSemCarne.setSelectedItem("Não");
	}
	...
	public void focoPadrao(){
		...
		// se colaborador que logou no sistema, não for o mesmo logado antes, inicializa a data de pagamento
		if(SaoDimasMain.ultimoColaborador != null && !SaoDimasMain.ultimoColaborador.getId().equals(vo.getId()))
		{
	...
	private void initialize() {
		...
		lbPagamentoSemCarne = new JLabel("Pagto. s/ Carne?:  ", JLabel.RIGHT);
		lbPagamentoSemCarne.setPreferredSize(DLABEL);
		lbPagamentoSemCarne.setMinimumSize(DLABEL);
				
		comboSemCarne = new JComboBox<String>();
		comboSemCarne.addItem("Sim");
		comboSemCarne.addItem("Não");

		comboSemCarne.setPreferredSize(DFIELDP);
		comboSemCarne.setMinimumSize(DFIELDP);
	...
	private void configure() {
		...
		c.gridy = 6; infFatura.add(comboSemCarne, c);
						...
						if(f.isSelecionada()){
							...
							try {
								...
								if(dataPagamento.after(new Date())){
									...
									f.setParceiro(null);
									f.setIsPagamentoSemCarne(comboSemCarne.getSelectedItem().toString().trim());
