...
public class ApoliceEditarFaturaPanel extends JPanel {
	...
	/*
	 * Verifica se a fatura selecionada é fatura paga sem carne
	...
	private void editarFatura() {
		...
		{
			JOptionPane.showMessageDialog(null, "Não é possível editar uma fatura já dada baixa.", "Atenção", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
	...
	private void excluirFaturas() {
		...
		FaturaVO primeiraFatura = (FaturaVO) model.getSelectedValue(idSelecao);
			...
			{
				JOptionPane.showMessageDialog(null, "Não é possível excluir uma fatura já dada baixa.", "Atenção", JOptionPane.INFORMATION_MESSAGE);
				return;
			}
			...
			if (c.mostraConfirmacao("Confirmação", mensagem, JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
	...
	private void validaSeFoiDadoBaixa(List<FaturaVO> listFaturasImprimir)throws MensagemSaoDimasException
	{
		for(FaturaVO vo : listFaturasImprimir){
			if(vo.getDataPagamento() == null || (vo.getIsPagamentoSemCarne() != null && !vo.getIsPagamentoSemCarne().equalsIgnoreCase("Sim")))
				throw new MensagemSaoDimasException("Só é permitido impressão de comprovante sem carne se a fatura já foi dada baixa no sistema e com opção sem carne marcada!");
		}
	}
	...
	private void destacadaBaixaFaturaPagaSemCarne() {
		...
		if (c.mostraConfirmacao("Confirmação", "Confirmar destaque do carne da fatura paga sem carne?",
	...
	private void estornarBaixaFatura() {
		...
		try {
			...
			FaturaTableModel model = (FaturaTableModel) tbFaturas.getModel();
			FaturaVO fat = ((FaturaVO) model.getSelectedValue(idSelecao));
			if (c.mostraConfirmacao("Confirmação","Confirma o estorno da baixa da fatura?", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
