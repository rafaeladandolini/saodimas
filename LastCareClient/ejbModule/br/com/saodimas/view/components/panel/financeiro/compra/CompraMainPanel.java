...
public class CompraMainPanel extends CustomLayeredPanel {
	...
	public CompraProdutoIFrame getIFrameCompraProduto() {
		return iframeCompraProduto;
	}
	...
	private void initialize() {
		...
		getDesktop().add(iframeCompraProduto);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btNovo){
				...
				if (mostraConfirmacao("Confirmação", "Confirma a exclusão da compra?", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION){
					int row = tbCompras.getSelectedRow();
					CompraTableModel model = (CompraTableModel)tbCompras.getModel();
					CompraVO compra = (CompraVO)model.getSelectedValue(row);
					try{
						Controller.getInstance().deleteCompra(compra);
