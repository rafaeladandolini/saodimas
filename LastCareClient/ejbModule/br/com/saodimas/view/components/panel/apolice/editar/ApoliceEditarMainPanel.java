...
import br.com.saodimas.view.components.iframe.CarteirinhasQtdeIFrame;
...
import br.com.saodimas.view.components.panel.apolice.editar.emprestimo.devolucao.ApoliceEditarDevolucaoEquipamentoPanel;
...
public class ApoliceEditarMainPanel extends CustomLayeredPanel {
	...
	private CarteirinhasQtdeIFrame iframeCarteirinhaQuantidade;
	...
	private ApoliceEditarDevolucaoEquipamentoPanel devolucaoEquipamento;
	...
	public CarteirinhasQtdeIFrame getIframeCarteirinhaQuantidade() {
		return iframeCarteirinhaQuantidade;
	}
	...
	public ApoliceEditarDevolucaoEquipamentoPanel getDevolucaoEquipamentoApoliceMainPanel() {
	...
	public void mostrarMensagemDevolucao(String msg, int tipo){
		this.devolucaoEquipamento.notificar(msg, tipo);
	}
	...
	{
		this.devolucaoEquipamento.carregarDevolucoes();
	}
	...
	public void setApolice(ApoliceVO apolice) {
		...
		this.devolucaoEquipamento.setApolice(apolice);
	...
	private void carregarDadosApolice()
	{
		...
		if(!apolice.getStatus().trim().equalsIgnoreCase("Ativo"))
			this.mensagemAlertaPlanoCanceladoSuspenso();
	...
	{
		labelApoliceSuperior.setText("                        "+ apolice.getPlano().getDescricao() + "  -  " + apolice.getNumeroContrato() +  "  -  "+ apolice.getTitular().getNome() + "                                              Ob.: " + apolice.getQtdeObitos() + " , Emp.: "+  apolice.getQtdeEmprestimos()+"  , Dev.: " + apolice.getQtdeDevolucoes() );
	}
	
	private void mensagemAlertaPlanoCanceladoSuspenso()
	{
		String mensagem = "Favor verificar a situação do plano: " + apolice.getStatus().toUpperCase(); 
	...
	private void initialize() {
		...
		iframeCarteirinhaQuantidade= new CarteirinhasQtdeIFrame(){
			@Override
			public void setVisible(boolean flag) {
				if (getGlass() != null) getGlass().setVisible(flag);
				super.setVisible(flag);
		...
		iframeCarteirinhaQuantidade.setLocation(x - iframeCarteirinhaQuantidade.getSize().width / 2, y - iframeCarteirinhaQuantidade.getSize().height / 2);
		...
		getDesktop().add(iframeCarteirinhaQuantidade);
		...
		apoliceTabPanel.addTab("Dados Apólice", new ImageIcon("imagens/apolice.gif"), apolicePanel);
		apoliceTabPanel.addTab("Titular/Responsável", new ImageIcon("imagens/titular.gif"), titularPanel);
		apoliceTabPanel.addTab("Contato/Endereço", new ImageIcon("imagens/home.gif"), contatoPanel);
		apoliceTabPanel.addTab("Inf. Sist. Antigo", new ImageIcon("imagens/apolice.gif"), infoSistemaAntigo);
		...
		devolucaoEquipamento = new ApoliceEditarDevolucaoEquipamentoPanel();
		...
		detalhesTabPanel.addTab("Devoluções", new ImageIcon("imagens/relacaoTitular.gif"), devolucaoEquipamento);
		...
		btImprimirCarteirinhas = new JButton("Impressão Carteirinhas",new ImageIcon("imagens/equipamento.gif"));
	...
	private class EventoBotaoComando implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			} catch (MensagemSaoDimasException ee) {
				barNotificacao.mostrarMensagem(ee.getMessage(), BarraNotificacao.ERRO);
				ee.printStackTrace();
			}
		}
	}
