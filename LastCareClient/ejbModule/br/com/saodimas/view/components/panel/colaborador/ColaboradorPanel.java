...
public class ColaboradorPanel extends JPanel {
	...
	private JRadioButton rdFuncEspecial;
...
//	private ErrorButton btErroRSenha;
	...
	public void setColaborador(ColaboradorVO colaborador) {
		...
		if(colaborador != null){
			...
			{
				rdAdmin.doClick();
			}else if(colaborador.getTipoColaborador().equals(rdFuncEspecial.getText())){
				rdFuncEspecial.doClick();
			}else
			{
				rdFunc.doClick();
			}
	...
	private void initialize() {
		...
		rdFuncEspecial = new JRadioButton("Funcionário Especial");
		rdFuncEspecial.addActionListener(new EventoRadioTipo());
		
		bgrTipo = new ButtonGroup();
		bgrTipo.add(rdFunc);
		bgrTipo.add(rdFuncEspecial);
		bgrTipo.add(rdAdmin);
		bgrTipo.setSelected(rdFunc.getModel(), true);
	...
	private class EventoRadioTipo implements ActionListener{
		public void actionPerformed(ActionEvent ev) {
			boolean isFuncionario = (ev.getSource() == rdFunc || ev.getSource() == rdFuncEspecial );
	...
	private class EventoBotaoControle implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btCancelar){
				try{
					((ColaboradorMainPanel)c).getIframeColaborador().setVisible(false);
				}
			...
			if (e.getSource() == btSalvar)
			{
				...
				String tipoColaborador = rdAdmin.getText();
				
				if(rdFunc.isSelected())
					tipoColaborador = rdFunc.getText();
				else if(rdFuncEspecial.isSelected())
					tipoColaborador = rdFuncEspecial.getText();
				
				novocolaborador.setTipoColaborador(tipoColaborador);
				...
				try {
					if(colaborador != null){
						...
						((ColaboradorMainPanel)c).mostrarMensagem("Alterações efetuadas com sucesso.",	barNotificacao.INFO);
					}else
					{
						Controller.getInstance().insertColaborador(novocolaborador);
						((ColaboradorMainPanel)c).mostrarMensagem("Cadastro efetuado com sucesso.",	barNotificacao.INFO);
					}				
					((ColaboradorMainPanel)c).carregarColaboradoressTable();
					((ColaboradorMainPanel)c).getIframeColaborador().setVisible(false);
				} catch (MensagemSaoDimasException e1) {
					((ColaboradorMainPanel)c).mostrarMensagem(e1.getMessage(),	barNotificacao.INFO);
					e1.printStackTrace();
				}
