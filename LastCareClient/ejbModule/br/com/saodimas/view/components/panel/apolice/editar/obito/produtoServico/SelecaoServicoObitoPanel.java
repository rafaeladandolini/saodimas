...
public class SelecaoServicoObitoPanel extends JPanel {
	
	private static final String MENSAGEM_PADRAO = "   (*) Preenchimento obrigatório";
	
	private ServicoObitoVO servicoObito;
	...
	public SelecaoServicoObitoPanel() {
		initialize();
		configure();
	}

	public ServicoObitoVO getServico() {
		return servicoObito;
	}

	public void setServico(ServicoObitoVO servicoObito) {
	...
	private class EventoBotaoControle implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == bAdicionar) {
				...
				ServicoObitoVO servicoAdicionar = new ServicoObitoVO();
				...
				try {
					if (c.getClass() == ApoliceEditarMainPanel.class) {
						((ApoliceEditarMainPanel) c).adicionaServico(servicoAdicionar);
						((ApoliceEditarMainPanel) c).getIframeSelecaoServico().setVisible(false);
					}
				} catch (Exception ex) {
					ex.printStackTrace();
